...
/* dan brown, 1 apr 2015
 * we add a type for symbolic music represented as intervals from -11 
   semitones (just less than an octave) to +11 semitones (up just less than 
   an octave),  so hence over a 
   23-letter alphbet (A to W).  Longer intervals are folded into this range.
 */
static ESL_ALPHABET *create_music(void);
...
esl_alphabet_Create(int type)
{
  ...
  case eslMUSIC:  a = create_music(); break;
  default:    
...
create_music(void)
{
  ...
  /* Create the internal alphabet
   */
  if ((a = esl_alphabet_CreateCustom("ABCDEFGHIJKLMNOPQRSTUVW-0*~", 23, 27)) == NULL) return NULL;
  a->type = eslCOINS;
  ...
  /* There are no degeneracies in the music alphabet. */

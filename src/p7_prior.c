...
*/
  ...
  // for simplicity, we'll start with just a 1-parameter Dirichlet prior
   int num_comp = 1;
   static double defmq[1] =  { 1.0  };

   // this needs to be a 23-element array of parameters for the single component in the Dirichlet prior?


   static double defm[1][23] = 
     {0.000023,0.000144, 0.000766, 0.001874, 0.007614, 0.000851, 0.012032, 0.012156, 0.020036, 0.070044,  0.027279,  0.089606,  0.020533,  0.057976,  0.023580,  0.016060,  0.020304,  0.000636,  0.007277,  0.000766,  0.003788,  0.000696,  0.000022}; 

   // now we need to fill in the details for the transition parameters.
  ...
  /* These are the Dirichlet priors for the transitions.  I'm pretty sure that tm -> pq[0] is to allow us to have
     multiple components in the transitions (essentially, gap-prone and gap-averse regions?), but I don't see 
     evidence that the hmmer developers did this themselves...

     Probably we want to tune these.
   */
  pri->tm->pq[0]       = 1.0;
  pri->tm->alpha[0][0] = 2.0; // TMM
  pri->tm->alpha[0][1] = 0.1; // TMI
  pri->tm->alpha[0][2] = 0.1; // TMD  These make inserts and deletes, by default, 20 times less common than matches

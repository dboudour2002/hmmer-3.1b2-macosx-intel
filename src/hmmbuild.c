...
#define ALPHOPTS "--amino,--dna,--rna, --music"                         /* Exclusive options for alphabet choice */
...
/* Selecting the alphabet rather than autoguessing it */
  ...
  // addition for music data, 1 apr 2015, dan brown
  {"--music",     eslARG_NONE,   FALSE, NULL, NULL,   ALPHOPTS,    NULL,     NULL, "input alignment is musical intervals",                  2 },
...
usual_master(const ESL_GETOPTS *go, struct cfg_s *cfg)
{
  ...
  else if (esl_opt_GetBoolean(go, "--music"))   cfg->abc = esl_alphabet_Create(eslMUSIC);
  else                                          cfg->abc = NULL;
  
  // since (for music) we supply the alphabet, this *shouldn't* make for problems: what comes back from msafile_Open 
  // should still be the right alphabet (or, I suppose, an error)
...
mpi_master(const ESL_GETOPTS *go, struct cfg_s *cfg)
{
  ...
  // I am not adding the --music here, as I don't want to worry about parallel programming here, yet.
  else                                          cfg->abc = NULL;
...
output_result(const struct cfg_s *cfg, char *errbuf, int msaidx, ESL_MSA *msa, P7_HMM *hmm, ESL_MSA *postmsa, double entropy)
{
  ...
  if (msa == NULL)
  {
    // we're not adding the music alphabet here, because (I think!) this is printing out the weight of the sequences, 
    // and we don't weight the music sequences
